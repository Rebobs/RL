...
def calc_reward(snr, tput, loss, bler, gain):
...
ax_snr, ax_tput, ax_loss, ax_gain, ax_reward, ax_blank = axes.flat
...
ln_tput   = make_line(ax_tput,   '#4caf50', '',    'Throughput',  (0, 12))
ln_loss   = make_line(ax_loss,   '#f44336', '',    'Packet Loss', (0, 1.05))
ln_gain   = make_line(ax_gain,   '#ffeb3b', '',    'Gain',        (0, 3.5))
...
def update(_frame):
    ...
    if msg is None:
        return ln_snr, ln_tput, ln_loss, ln_gain, ln_reward
    ...
    reward = calc_reward(snr, tput, loss, bler, gain)
    ...
    status_text.set_text(
        f"#{step_counter[0]:5d} | SNR={snr:5.1f}dB | tput={tput:4.1f} | "
        f"loss={loss:.3f} | gain={gain:.2f} | reward={reward:6.2f}"
    )
